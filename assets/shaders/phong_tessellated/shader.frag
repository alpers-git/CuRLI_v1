...
vec3 illuminationAt(in PLight light, in vec3 pos, samplerCubeShadow shadow_map, in vec3 w_space_pos, inout vec3 l)
{
     ...
     {
          if(texture(shadow_map, vec4(l, l_length)) > l_length * 0.01)
               intensity = 0;
     }
...
//------------ Variying ------------
layout (location = 13) in vec3 v_space_norm;
layout (location = 14) in vec3 v_space_pos;
layout (location = 15) in vec2 tex_coord;
layout (location = 16) in vec3 w_space_pos;
layout (location = 17) in vec3 w_space_norm;
...
uniform samplerCubeShadow p_shadow_maps[5];
